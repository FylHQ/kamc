...
<v-container grid-list-xs fluid>
   <v-layout row wrap>
      <v-flex xs10 sm4>
         <upload source-type="smbusiness" name="Загрузить информацию о СМСП" v-on:upload-success="onSuccessUpload"></upload>
      </v-flex>
      <v-flex xs2 sm1>
         <v-progress-circular
      ...
      <v-flex xs12 sm7>
         <v-layout row wrap justify-space-between>
            ...
            <v-flex sm3>
               <v-checkbox
                  v-model="importSettings.enableCntr"
                  label="Обновить договоры"
               ></v-checkbox>
            </v-flex>
            <v-flex sm3>
               <v-checkbox
                  v-model="importSettings.enablePaym"
                  label="Обновить протоколы"
               ></v-checkbox>
            </v-flex>
         </v-layout>
      </v-flex>
...
export default {
   ...
   methods: {
      ...
      importSelected() {
         ...
         axios.post('/import', {importCode: 'smbusiness', codes: codes, settings: self.importSettings})
            .then(result => {
